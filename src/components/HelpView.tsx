...
/** Aba Ajuda: versão/atualização, guia de uso e como configurar o Spotify. */
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4">
      <div className="max-w-2xl space-y-6">
        {/* Versão / atualização */}
        <Section title="Sobre & Atualizações">
          ...
          {update && update.error && (
            <p className="mt-3 text-sm text-amber-400">
              Não foi possível verificar agora ({update.error}). Tente novamente ou veja em “Ver todas as versões”.
            </p>
          )}
          {update && !update.error && update.isNewer && (
            <div className="mt-3 rounded border border-emerald-800/60 bg-emerald-950/40 p-3 text-sm">
              <p className="text-emerald-300">🎉 Nova versão <strong>v{update.latest}</strong> disponível (você tem v{update.current}).</p>
          ...
          {update && !update.error && !update.isNewer && update.latest && (
            <p className="mt-3 text-sm text-emerald-400">✓ Você está na versão mais recente (v{update.latest}).</p>
          )}
        </Section>
        ...
        <Section title="Configurar o Spotify (Client ID e Client Secret)">
          ...
          <ol className="mt-3 list-decimal space-y-1.5 pl-5 text-sm text-neutral-300">
            ...
            <li>Clique em <strong>Create app</strong>. Preencha nome e descrição (quaisquer).</li>
            <li>Em <strong>Redirect URI</strong>, coloque <code className="rounded bg-neutral-800 px-1">http://localhost:8888/callback</code> (o formulário exige, mas não é usado) e marque <strong>Web API</strong>.</li>
            <li>Abra o app criado → <strong>Settings</strong> e copie o <strong>Client ID</strong> e o <strong>Client Secret</strong>.</li>
            <li>Cole em <strong>Configurações → Credenciais Spotify</strong> e clique em <strong>Salvar</strong>.</li>
          </ol>
